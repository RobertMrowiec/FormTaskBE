    ...
    it('should throw error during adding event to database without firstName', (done) => {
      chai.request(server)
          ...
          .end((err, res) => {
            ...
            res.body.message.should.equal('Incorrect form data')
            done();
          });
    });
    ...
    it('should save an object into events collection', (done) => {
      chai.request(server)
        ...
        .end((err, res) => {
          ...
          res.body.saved.firstName.should.equal('Test');
          res.body.saved.lastName.should.equal('Test2');
          res.body.saved.email.should.equal('asd@gmail.com');
          res.body.saved.date.should.equal(new Date('2018-04-02').toISOString());
          done();
        });
    });
  });
